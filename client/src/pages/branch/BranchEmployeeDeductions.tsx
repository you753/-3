...
export default function BranchEmployeeDeductions({ branchId }: { branchId: number }) {
  ...
  // تصفية الخصومات حسب البحث والنوع
  const filteredDeductions = useMemo(() => {
    return deductions.filter((deduction: any) => {
      ...
      const matchesType = filterType === 'all' || deduction.deductionType === filterType;
      return matchesSearch && matchesType;
    });
  }, [deductions, employees, searchTerm, filterType]);

  // حساب الصفحات
  const pageCount = Math.ceil(filteredDeductions.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, filteredDeductions.length);
  const paginatedDeductions = filteredDeductions.slice(startIndex, endIndex);

  // إعادة ضبط الصفحة عند تغيير البحث أو الفلتر
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={filterType} onValueChange={handleFilterChange}>
              ...
              <SelectContent>
                ...
                <SelectItem value="salary_to_debt">🔄 تحويل إلى دين</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardContent>
